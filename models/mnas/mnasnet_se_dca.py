...
__all__ = [ 'dca_se_mnasnet0_5', 'dca_se_mnasnet0_75', 'dca_se_mnasnet1_0','dca_se_mnasnet1_0r8', 'dca_se_mnasnet1_3']
    ...
    def __init__(self, in_channel,channel, reduction = 16):
        ...
        if in_channel != channel:
            self.att_fc = nn.Sequential(
                nn.Linear(in_channel, channel // reduction),
...
class _InvertedResidual(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, kernel_size, stride, expansion_factor,
        ...
        self.se = SELayer(in_ch,out_ch,reduction=reduction)

    def forward(self, x):
        input = x[0]
        pre_att = x[1]
        se_out = self.se({0: self.layers(input), 1: pre_att})
        if self.apply_residual:
            return {0: input + se_out[0], 1: se_out[1]}
        else:
            return se_out
...
def dca_se_mnasnet0_5(pretrained=False, progress=True, **kwargs):
...
def dca_se_mnasnet0_75(pretrained=False, **kwargs):
...
def dca_se_mnasnet1_0(pretrained=False, **kwargs):
...
def dca_se_mnasnet1_0r8(pretrained=False, reduction=8,**kwargs):
    """MNASNet with depth multiplier of 1.0 from
    `"MnasNet: Platform-Aware Neural Architecture Search for Mobile"
    <https://arxiv.org/pdf/1807.11626.pdf>`_.
    Args:
...
def dca_se_mnasnet1_3(pretrained=False, **kwargs):
...
def demo():
    net = dca_se_mnasnet1_0r8(num_classes=1000)
