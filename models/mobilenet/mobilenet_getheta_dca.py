...
__all__ = ['dca_getheta_mobilenet_v2']
...
class InvertedResidual(nn.Module):
    def __init__(self, inp, oup, stride, expand_ratio):
        ...
        self.ge = GEBlock(inp,oup)

    def forward(self, x):
        out = self.ge({0:self.conv(x[0]),1:x[1]})
        if self.use_res_connect:
            return {0:x[0]+out[0],1:out[1]}
        else:
            return out
...
def dca_getheta_mobilenet_v2(pretrained=False, **kwargs):
    """
...
def demo():
    net = dca_getheta_mobilenet_v2(num_classes=1000)
