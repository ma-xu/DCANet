...
class RContextBlock2d(nn.Module):

    def __init__(self,preplanes, inplanes, planes, pool='att', fusions=['channel_add'],reduction = 16):
        super(RContextBlock2d, self).__init__()
        ...
        if preplanes!=inplanes:
            self.att_fc = nn.Sequential(
                nn.Conv2d(preplanes, self.planes, kernel_size=1),
                nn.LayerNorm([self.planes, 1, 1]),
                nn.ReLU(inplace=True),
                nn.Conv2d(self.planes, self.inplanes, kernel_size=1)
            )
    ...
    def forward(self, x):
        # [N, C, 1, 1]
        context = self.spatial_pool(x[0])
        all_context = context
        # [N, C, 1, 1]
        if x[1] is not None:
            pre_att = self.att_fc(x[1]) if hasattr(self, 'att_fc') else x[1]
            pre_att = self.weight * pre_att
            all_context = pre_att + all_context
        channel_add_term = self.channel_add_conv(all_context)
        out = x[0] + channel_add_term

        return {0:out,1:channel_add_term}
...
class Bottleneck(nn.Module):
    ...
    def __init__(self,
        ...
        # self.gc = RContextBlock2d(planes * self.expansion, planes // 4, 'att',  ['channel_add'])
...
class ResNetGCDCA(nn.Module):
    ...
    def __init__(self,
        ...
        super(ResNetGCDCA, self).__init__()
    ...
    def train(self, mode=True):
        super(ResNetGCDCA, self).train(mode)
